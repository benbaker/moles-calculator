...
function doTheThing() {
  ...
  if (grams == "") { $("#grams").val(1); }
  
  //var moles = $("#moles").val();
  var inputMolecule = $("#element").val();

  if (inputMolecule.length >0 ){
  ...
  var errorBox = "<div class='moleculeBox'><p0>Invalid Formula<br />" + " </p0><p2>Computer Science Fags can't chemistry." + "</p2><p3> </p3><p2><br />";
...
function processMolecule() {
  ...
  var resultBoxMolecule = "<div class='moleculeBox'><p0>" + $("#element").val() + " </p0><p2>" + r(molesOfMolecule) + "</p2><p3> moles </p3><p2>" + "" + $("#grams").val() + "</p2><p3> grams </p3><br />";
...
function gramsToMoles(inputMolecule, grams) {
  ...
  for (i in theElements) {
    ...
    if (theElements[i].symbol === inputMolecule) {
      ...
      var moles = (Math.round( (grams / theElements[i].atomicMass) * 10000) ) / 10000;
...
function makeTable() {
  ...
  var i = 0;
    while ( i < 19) {
      ...
      $(".table").append("<div id='" + i + "' style='width:26px;float:left;''></div>");
      i++
      
      
    }

    //$(".table").append("<div id='" + 101 + "' style='clear:both'></div>");
    //$(".table").append("<div id='" + 102 + "' style='clear:both'></div>");
  ...
  for (i in theElements) {
    ...
    var tableItem="<div class='tableElement' symbol='" + theElements[i].symbol + 
    "' style='float:left;margin-left:1px;padding:1px;padding-top:8px;font-size:12px;font-weight:900;line-height:8px;margin-top:1px;background-color:" +
        color + ";color:#000;width:23px;opacity:0.8'><p><span style='color:#fff'>" +
        "</span>" + theElements[i].symbol + "</p></span>";
    ...
    $("#" + group).append(tableItem);
      ...
      $(".tableElement").click(function() {
		...
		$("#element").val(elementValue + $(this).attr("symbol"));
		$("#element").focus();
...
$(document).ready(function(){
	...
	if (whereFocus == "grams") {
			...
			if (gramsvalue > 0) {
				...
				$("#grams").val(value + this.innerHTML);
				$("#moles").val("");
				$("#grams").focus();
				$(".lastValue").html(value + this.innerHTML);
	...
	if (whereFocus == "moles") {
			...
			if (value > 0) {
				...
				$(".lastValue").html(value + this.innerHTML);
				$("#moles").val(value + this.innerHTML)
				$("#grams").val("");
    ...
    $('.numPadButton').click(function(){
        ...
        if(this.innerHTML == 'DEL'){
            var gramsValue = $("#grams").val();
            if(gramsValue.length > 0){
                gramsValue = gramsValue.substring(0, gramsValue.length - 1);
                $("#grams").val(gramsValue);
        ...
        if(this.innerHTML == 'DEL'){
            var gramsValue = $("#moles").val();
            if(gramsValue.length > 0){
                gramsValue = gramsValue.substring(0, gramsValue.length - 1);
                $("#moles").val(gramsValue);
            }
        }
        else{
